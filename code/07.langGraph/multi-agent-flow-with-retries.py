...
def code_accuracy_check(state: MessageState):
    ...
    if state["fix_accuracy_retries"]>0:
        SYSTEM_PROMPT += "\n The accuracy is less. Write 100% accurate code"
...
def is_accuracy_greater_than_threshold(state: MessageState) -> Literal["coding_query", END]:
    print(f"🎶 is_accuracy_greater_than_threshold: check num -> {state["fix_accuracy_retries"]}+1")
    accuracy_threshold = 95
    
    if state["code_accuracy"] < accuracy_threshold and state["fix_accuracy_retries"] < 5:
        return "coding_query"
    return END
...
graph_builder.add_node("is_accuracy_greater_than_threshold", is_accuracy_greater_than_threshold)
...
graph_builder.add_conditional_edges("code_accuracy_check", is_accuracy_greater_than_threshold)
...
if __name__=="__main__":
    ...
    with open(f"{path}/multi-agent-flow-with-retries.png","wb") as f:
        f.write(png)
